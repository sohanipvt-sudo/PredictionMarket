...
export const CONTRACT_ADDRESS =
  "CBUI7FDF57JBGPFPI7CYVZSVTIJR35CPCGUA52I3XMPELDGMWDKRS45K";
 ...
 * Calls: init()
 */
export async function initMarket(caller: string) {
  return callContract("init", [], caller, true);
}

/**
 * Place a bet on the market.
 * Calls: place_bet(user: Address, amount: i128, prediction: bool)
 * @param caller - The user's wallet address
 * @param amount - Bet amount in XLM (as i128/scaled)
...
export async function placeBet(
  ...
  return callContract(
    "place_bet",
    [toScValAddress(caller), toScValI128(amount), toScValBool(prediction)],
    caller,
    true
  );
}
 ...
 * Calls: resolve(outcome: bool)
 * @param outcome - true = YES won, false = NO won
 */
export async function resolveMarket(caller: string, outcome: boolean) {
  return callContract(
    "resolve",
    [toScValBool(outcome)],
    caller,
 ...
 * Calls: claim(user: Address) -> i128
 * @returns The reward amount
 */
export async function claimReward(caller: string) {
  return callContract(
    "claim",
 ...
 * Calls: get_market() -> Market
 * Returns: { resolved: bool, outcome: bool, bets: [] }
 */
export async function getMarket(caller?: string) {
  return readContract("get_market", [], caller);
}
